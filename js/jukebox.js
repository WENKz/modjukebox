...
function createTiles(qtype) {
    return createTileCircle(qtype, 420);
}

function createTileCircle(qtype, radius) {
    var start = now();
    var y_padding = 0;
    var x_padding = 18; //old:5
    var maxWidth = 90; //old:90
    var tiles = [];
...
//  var perimeter = 2 * n * R * Math.sin(alpha/2);
    ...
    for (var i = 0; i < qlist.length; i++) {
        var tile = createNewTile(i, qlist[i], width, a);
        var y = y_padding + R;
        var x = x_padding + ( 2 * R / qlist.length ) * i;
        tile.move(x, y);
   //     tile.rotate(angle);
        tiles.push(tile);
        angle += alpha;
    ...
    // geometry
    // this is the beginning of line stuff
    var roffset = width * 1;
 ...
 //   var center = ' S 450 0 '
  ...
  //  R -= roffset;
    for (var i = 0; i < tiles.length; i++) {
     //   var startAngle = i + x_padding;
        var tile = tiles[i];
        var y1 = y_padding + R;
        var x1 = x_padding + ( 2 * R / tiles.length ) * i;


// x1/y1 appears to refer to the beginning point of the curve. x2/y2 appears to represent the ending point of the curve

        for (var j = 0; j < tile.q.neighbors.length; j++) {
//        var destAngle = x_padding + tile.q.neighbors[j].dest.which;
        var y2 = y_padding + R;
        var x2 = x_padding + ( 2 * R / tiles.length ) * tile.q.neighbors[j].dest.which;

 //         var x2 = startAngle + R * Math.cos(destAngle);
var xc = ( x1 + x2 ) / 2;
var yc = 50;
          var path = 'M' + x1 + ' ' + y1 + ' ' + 'S' + ' ' + xc + ' ' + yc + ' ' + x2 + ' ' + y2; //set beginning curve x1, ??? (something), y1 to center then to x2, ??? (something), y2
 // old         var path = 'M' + x1 + ' ' + y1 + center + x2 + ' ' + y2;
...
function trackReady(t) {
    ...
    document.title = 'Pathways for ' + t.fixedTitle;
